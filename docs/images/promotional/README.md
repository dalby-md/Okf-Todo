# OKF-Todo screenshot gallery

[Return to the project README](../../../README.md)

These screenshots use fictional customer and task data. They were captured from the current production UI at 1280 × 720 and are displayed without forced dimensions.

## Light interface

### 1. Choose HTML or Markdown

![General Preferences page with HTML and Markdown editor choices](00-preferences-editor-mode.png)

Choose HTML or Markdown as the default editor for viewing and editing task notes.

### 2. Light workspace

![OKF-Todo light workspace with task list, task metadata, tags, and rich-text editor](01-workspace-light.png)

The main workspace combines task views and search with a detailed task editor.

### 3. Task workflow

![Task relationships, checklist, attachments, and timeline](02-task-workflow.png)

Relationships, checklist progress, attachments, comments, and automatic history remain together with the task.

### 4. Task relationships

![Task relationships with linked tasks and relationship controls](11-task-relationships.png)

Typed relationships connect related work while keeping checklist progress and timeline context close by.

### 5. Task details preferences

![Task details preference page with layout and field visibility controls](09-preferences-task-details.png)

Task layout, source fields, and relationship visibility are grouped on a dedicated Preferences page.

### 6. Data and values

![Data and values preference page](05-preferences-data-values.png)

Task types, priorities, statuses, and tags have a dedicated Preferences page.

### 7. Backup

![Database backup preference page](06-preferences-backup.png)

Backup is isolated on its own focused Preferences page.

## Help

### 8. Help overview

![Offline Help with OKF and MCP topics](10-help-overview.png)

Offline Help keeps the OKF and MCP user guides available directly inside the desktop application.

### 9. OKF layer Help

![End-user OKF layer Help guide](03-help-okf-layer.png)

The offline OKF guide explains how an AI harness can turn source material into reviewed artifacts.

### 10. MCP server Help

![End-user MCP server Help guide](04-help-mcp-server.png)

The MCP guide explains the optional local action bridge and its review-before-save workflow.

## Dark interface

### 11. Dark-mode preference

![Appearance preference page with dark mode selected](07-preferences-appearance-dark.png)

The Appearance page provides an immediate light or dark color-scheme choice.

### 12. Dark workspace

![OKF-Todo dark workspace with rich-text editing](08-workspace-dark.png)

The full workspace remains readable and consistent in dark mode.

[Return to the project README](../../../README.md)
